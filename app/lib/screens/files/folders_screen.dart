...
class _FoldersScreenState extends State<FoldersScreen> {
  ...
  Future<void> _loadFolders() async {
    ...
    try {
      // Get folder counts from Firestore
      ...
      for (var doc in querySnapshot.docs) {
        ...
        if (data != null) {
          ...
          // Add custom folders to list if not already there
  ...
  Future<void> _deleteFolder(String folderName) async {
    ...
    try {
      // Move all documents from this folder to General
      final querySnapshot = await _firestoreService.getCollection('documents');
      for (var doc in querySnapshot.docs) {
        final data = doc.data() as Map<String, dynamic>?;
      ...
      setState(() {
        ...
        _folderDocumentCounts['General'] = 
            (_folderDocumentCounts['General'] ?? 0) + 
            (_folderDocumentCounts[folderName] ?? 0);
      });

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Folder "$folderName" deleted. Documents moved to General.'),
          backgroundColor: Colors.green,
        ),
      );
    } catch (e) {
          ...
          : _folders.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final count = _folderDocumentCounts[folderName] ?? 0;
